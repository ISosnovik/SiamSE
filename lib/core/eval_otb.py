...
def get_result_bb(arch, seq):
    result_path = os.path.join(arch, seq + '.txt')
...
def eval_auc(dataset='OTB2015', result_path='./test/', tracker_reg='S*', start=0, end=1e6, dataset_root=None):
    if dataset_root:
        list_path = os.path.join(dataset_root, dataset + '.json')
    else:
        raise FileNotFoundError
    annos = json.load(open(list_path, 'r'))
    seqs = list(annos.keys())  # dict to list for py3

    trackers = glob.glob(os.path.join(result_path, dataset, tracker_reg))
    ...
    for i in range(n_seq):
        ...
        for j in range(len(trackers)):
            ...
            print('-- {:<6}| {:<20}| {:0.3f}|'.format(i, seq, success_overlap[i][j].mean()))
...
# ------------------------------------------------------------------------------------------------------------------
def eval_auc_tune(result_path, dataset='OTB2015', dataset_root=None, max_frames=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tracker_reg', type=str, required=True)
